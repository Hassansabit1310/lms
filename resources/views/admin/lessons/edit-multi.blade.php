...
Matter.World.add(currentEngine.world, [ground, ...balls]);"></textarea>
                ...
                case 'quiz':
                    ...
                    inputElement = `
                        <div class="space-y-4">
                            <div class="flex items-center justify-between">
                                <span class="text-sm font-medium text-gray-700">Select Quiz</span>
                                <a href="{{ route('admin.quizzes.create') }}" target="_blank" class="text-indigo-600 hover:text-indigo-800 text-sm font-medium">
                                    ➕ Create New Quiz
                                </a>
                            </div>
                            ...
                            <div class="space-y-3">
                                <div class="quiz-loading hidden">
                                    <div class="flex items-center space-x-2 text-gray-600">
                                        ...
                                        <span class="text-sm">Loading quizzes...</span>
                                    </div>
                                </div>
                                
                                <div class="quiz-selection">
                                    <select name="content_blocks[${blockIndex}][content]" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 quiz-select">
                                        <option value="">Select a quiz...</option>
                                    </select>
                                    <button type="button" onclick="loadQuizzesForBlock(${blockIndex})" class="mt-2 px-3 py-1 bg-blue-500 text-white text-sm rounded hover:bg-blue-600">
                                        🔄 Reload Quizzes
                                    </button>
                                </div>
                                
                                <div class="quiz-preview hidden">
                                    <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                                        <h4 class="font-medium text-gray-900 quiz-title"></h4>
                                        <p class="text-sm text-gray-600 quiz-description mt-1"></p>
                                        <div class="flex items-center space-x-4 mt-2 text-xs text-gray-500">
                                            <span class="quiz-type"></span>
                                            <span class="quiz-questions"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    `;
                    // Load quizzes after creating the element
                    setTimeout(() => loadQuizzesForBlock(blockIndex), 100);
                    break;
            }
        ...
        async function loadQuizzesForBlock(blockIndex) {
            const courseId = {{ $course->id }}; // Get course ID from the passed course object
            console.log('loadQuizzesForBlock called with:', { blockIndex, courseId });
            
            if (!courseId) {
                console.log('No course ID available, returning early');
                return;
            }

            const block = document.querySelectorAll('.content-block')[blockIndex];
            const loadingDiv = block.querySelector('.quiz-loading');
            const selectElement = block.querySelector('.quiz-select');
            const previewDiv = block.querySelector('.quiz-preview');

            console.log('Block elements found:', { block, loadingDiv, selectElement, previewDiv });

            try {
                loadingDiv.classList.remove('hidden');
                
                const url = `{{ route('admin.api.quizzes-for-lesson') }}?course_id=${courseId}`;
                ...
                selectElement.innerHTML = '<option value="">Select a quiz...</option>';
                
                // Add quiz options
                if (data.quizzes && data.quizzes.length > 0) {
                    data.quizzes.forEach(quiz => {
                        console.log('Adding quiz option:', quiz);
                        const option = document.createElement('option');
                        option.value = quiz.id;
                        option.textContent = quiz.title;
                        option.dataset.description = quiz.description || '';
                        option.dataset.type = quiz.quiz_type;
                    ...
                    const option = document.createElement('option');
                    option.value = '';
                    option.textContent = 'No quizzes available for this course';
                    selectElement.appendChild(option);
                }
                ...
                selectElement.innerHTML = '<option value="">Error loading quizzes</option>';
            } finally {
                loadingDiv.classList.add('hidden');
            }
        }

        function showQuizPreview(previewDiv, quiz) {
            const titleElement = previewDiv.querySelector('.quiz-title');
            const descriptionElement = previewDiv.querySelector('.quiz-description');
            const typeElement = previewDiv.querySelector('.quiz-type');

            titleElement.textContent = quiz.title;
            descriptionElement.textContent = quiz.description || 'No description available';
            typeElement.textContent = quiz.type.replace('_', ' ').toUpperCase();

            previewDiv.classList.remove('hidden');
        }

        // Load quizzes when course selection changes
        document.addEventListener('change', function(e) {
            if (e.target.name === 'course_id') {
                // Reload quizzes for all quiz blocks
                document.querySelectorAll('.content-block').forEach((block, index) => {
                    ...
                    if (select && select.value === 'quiz') {
                        loadQuizzesForBlock(index);
                    }
                });
            }
        });
